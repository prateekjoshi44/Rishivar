...
const IncomingCall = () => {
    ...
    const handleRejectCall = async (id) => {
        try {
            patchCallStatus({ id, status: "UserRejected" })
    ...
    if (callsRes.isLoading) return <></>
    if (callsRes.isError) return <ApiErrorModal res={callsRes} />

    const calls = callsRes.data
    const requestedCall = calls.find(i => i.status === "Accepted")
    ...
    if (requestedCall)
        return (
            ...
            <div className='d-flex w-100 bg-white text-dark align-items-center p-3 p-lg-3 border-bottom rounded-bottom-3 shadow-sm'>
                <p className='flex-grow-1'>{requestedCall.astro.name} accepted your call Request</p>
